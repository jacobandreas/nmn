...
def load_images(set_name):
  ...
  with open("data/images/processed/%s.query" % set_name) as query_f, \
    ...
    for i in range(inputs.shape[0]):
      print hash(str(inputs[i,:,:,:].nonzero()))

    #images = dict()
    #for image_id, index in i2i.items():
    #  print "[", image_id, index, hash(str(inputs[index,:,:,:].nonzero())), "]"
    #  images[image_id] = inputs[index,:,:,:]

    questions = dict()
    #for question_id, query in enumerate(query_f, start=1):
    for question_id, query in zip(question_ids, query_f):
      questions[question_id] = parse_query(query.strip())

    #for answer_id, answer in enumerate(output_f, start=1):
